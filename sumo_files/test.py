...
class Parameter:
    ...
    def __init__(self, name, frequency, vehicle_id) -> None:
    ...
    def start_timer(self):
        timer = threading.Timer(self.frequency, self.monitor)
        timer.start()
    ...
    def monitor(self):
        ...
        # self.vehicle_id = vehicles[0]

        self.x, self.y = traci.vehicle.getPosition(self.vehicle_id)
        self.coord = [self.x, self.y]
        self.lon, self.lat = traci.simulation.convertGeo(self.x, self.y)
        self.gpscoord = [self.lon, self.lat]
        self.edge = traci.vehicle.getRoadID(self.vehicle_id)
        self.lane = traci.vehicle.getLaneID(self.vehicle_id)
        self.displacement = round(
            traci.vehicle.getDistance(self.vehicle_id), 2)

        if (self.parameter_name == "speed"):
            
            self.value = round(traci.vehicle.getSpeed(self.vehicle_id)*3.6, 2)
            print("speed", self.value, "vehicle id", self.vehicle_id)
        elif (self.parameter_name == "turn_angle"):
            print(traci.vehicle.getIDCount())
            self.value = round(traci.vehicle.getAngle(self.vehicle_id), 2)
            print("turn Angle", self.value, "vehicle id", self.vehicle_id)
        self.start_timer()
...
class Vehicle:
    ...
    def __init__(self, id) -> None:
        ...
        self.edge = None
        self.lane = None
        self.spd = Parameter("speed", 10, id)
        self.turnAngle = Parameter("turn_angle", 5, id)


# traci.vehicle.add("veh_ai",  typeID="veh_passenger")
